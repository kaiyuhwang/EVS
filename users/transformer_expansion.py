...
class TransformerModelBaseExpansion(FairseqEncoderDecoderModel):
    """
    ...
    def load_state_dict(
        ...
        new_vocab_size = self.encoder.embed_tokens.num_embeddings-load_state_num_embeddings
        addition_tensor = torch.zeros(new_vocab_size, self.encoder.input_dim)
        nn.init.normal_(addition_tensor, mean=0, std=5 ** -0.5)
        for k in state_dict:
            if state_dict[k].size()[0] == load_state_num_embeddings:
                state_dict[k] = torch.cat((state_dict[k], addition_tensor), dim=0)
